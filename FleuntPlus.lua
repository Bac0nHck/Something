...
-- Скрытый родитель для UI: gethui() -> get_hidden_gui() -> CoreGui -> PlayerGui (фолбэк).
-- Так окно не висит в PlayerGui, где его видит сервер/анти-чит.
local GuiParentGetters = {
	function()
	...
	function()
		...
		return Env and Env.gethui and Env.gethui()
	end,
	function()
...
local function AttachGui(Gui)
	for _, GetParent in ipairs(GuiParentGetters) do
		local Ok, Parent = pcall(GetParent)
		if Ok and typeof(Parent) == "Instance" then
			-- Parent читаем обратно внутри pcall: у скрытых контейнеров он может
			-- вернуться как nil — это не ошибка, значит присвоение прошло.
			local Applied, Current = pcall(function()
				Gui.Parent = Parent
				return Gui.Parent
			end)
			if Applied and (Current == Parent or Current == nil) then
				return Parent
			end
		end
	end

	-- ни один защищённый контейнер не доступен — работаем как раньше
...
Library.GUI = GUI
ProtectGui(GUI)
Library.GuiParent = AttachGui(GUI)
