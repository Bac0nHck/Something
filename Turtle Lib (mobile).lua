...
-- Pointer position that follows BOTH mouse and touch (mobile fix).
-- InputObject.Position is in the same coordinate space as mouse.X/mouse.Y,
-- so on PC this is identical to the mouse and on phone it tracks the finger.
local UserInputService = game:GetService("UserInputService")
local pointer = Vector2.new(mouse.X, mouse.Y)
	    ...
	    obj.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                con = stepped:Connect(function()
        			if isdragging then
						local delta = Vector3.new(pointer.X, pointer.Y, 0) - minitial;
        ...
        function SliderMovement(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                isdragging = true;
                    ...
                    con = stepped:Connect(function()
                        if isdragging then
                            local xOffset = pointer.X - delta1 - 3
                            if xOffset > 175 then
                                xOffset = 175
    ...
    function functions:ColorPicker(name, default, callback)
        ...
        Canvas.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                con = stepped:Connect(function()
                    if isdragging then
                        local delta2 = Vector2.new(pointer.X, pointer.Y) - delta
                        local x = math.clamp(delta2.X, 2, Canvas.Size.X.Offset - 2)
                        local y = math.clamp(delta2.Y, 2, Canvas.Size.Y.Offset - 2)

				        sat = 1 - math.clamp((pointer.X - canvasPosition.X) / canvasSize.X, 0, 1)
				        brightness = 1 - math.clamp((pointer.Y - canvasPosition.Y) / canvasSize.Y, 0, 1)
        ...
        Color.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                if draggingColor then
                    ...
                    hue = 1 - math.clamp(1 - ((pointer.X - colorPosition.X) / colorSize.X), 0, 1)
                    ...
                    local xOffset = math.clamp(pointer.X - delta1, 0, Color.Size.X.Offset - 3)
